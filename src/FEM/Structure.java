...
public class Structure {
	...
	public void solveArclengthControl(int step) {
		...
		while (n < step) {
			...
			this.u = u_n.toArray();
			u_n.print("Displacement of step n");
		}
	...
	public Vector computeDur(Vector displacement, Vector r_external,
		...
		r_int.assign(this.assembleInternalForce(u));
	...
	public Vector assembleInternalForce(Vector displacement) {
	...
	public void solveNewtonRaphson(int step, double loadIncrement) {
		int DOFs = this.enumerateDofs();
		double[] r_external = new double[DOFs];
		...
		while (n < step) {
			...
			Vector u_k = new Vector(DOFs);
			u_k.assign(u_n);
			while (k < 40 && convergence > eps) {

				Vector r_int = new Vector(DOFs);
				r_int.assign(this.assembleInternalForce(u_k));
				IMatrix Kt = new Array2DMatrix(DOFs, DOFs);
				this.assembleTangentStiffnessMatrix(Kt, u_k);
				Vector rhs = r_ext.add(-lamda_n, r_int);
				Vector delta_u = this.solveLinearSystemEquation(Kt, rhs);
				u_k = delta_u.add(1, u_k);
				Vector difference = u_n.add(-1, u_k);
				convergence = delta_u.Norm2() / difference.Norm2();
				System.out.println("convergence: " + convergence);
				k = k + 1;
				System.out.println("k = " + k);
			}
			u_n.assign(u_k);
			lamda_n = lamda_n + loadIncrement;
			n = n + 1;
		}
		this.selectDisplacement(u_n.toArray());
	}
